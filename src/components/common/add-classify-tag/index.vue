<template>
	<div v-if="isShow">
		<el-dialog title="添加分类" :close-on-click-modal="false" size="large" custom-class="mod-pop" top="10%" :visible.sync="isShow" :modal="false">
...
<script>
	...
	export default {
		...
		methods:{
			formatSels(){
				...
				for(var k in this.sels){
					if(this.sels[k].id != undefined || this.sels[k].outCateId != undefined){
						ar.push(this.sels[k]);	
					}else{
						ar.push({id: this.sels[k], outCateId: this.sels[k]});	
					}
				}	
				this.sels = ar;
			},
			onSubmit(){
				...
				this.bus.$emit('classify/submit', this.sels);
				this.isShow = false;
			},
			getRedistable(){
				let self = this;
			...
			init(obj){
				...
				if(obj.tableId){
					...
					if(obj.sels.length){
						let productArr = util.array.toObject(obj.sels, 'id');
		...
		mounted(){
			var self = this;
			this.bus.$on('addClassify', function(obj){
				self.opt = obj;
				self.init(obj);
			});
