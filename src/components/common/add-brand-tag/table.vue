...
<script>
	...
	export default {
		...
		methods:
		{
			...
			selsChange(sels, row)
			{
				if(this.sels[row.outBrandId]){
					delete this.sels[row.outBrandId]
				}else{
					if(this.selsCache[row.outBrandId]){
						this.sels[row.outBrandId] = this.selsCache[row.outBrandId];
					}else{
						this.sels[row.outBrandId] = row
					}
				}
			...
			selsChangeAll(sels){
				this.tableData && this.tableData.forEach(item=>{
					if(sels.length==0){
						this.sels[item.outBrandId] && (delete this.sels[item.outBrandId]);	
					}else{
						if(this.selsCache[item.outBrandId]){
							this.sels[item.outBrandId] = this.selsCache[item.outBrandId];
						}else{
							this.sels[item.outBrandId] = item;
						}
					}
				})	
				this.bus.$emit('table/cols', this.sels);
			...
			selectedOption(){
				...
				if(rows && rows.length!=0){
					rows.forEach(row => {
						 if(selectedRows && selectedRows[row.outBrandId]){
							 this.checked[row.outBrandId] = row;
			...
			getData()
			{
				...
				self.$ajax.post(self.$apiUrl.marketing.getBrandList,opt
				).then((res) =>
				{
