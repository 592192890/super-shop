    ...
    <div class="mod-block mod-set">
			...
			<el-row type="flex" class="mod-tips" justify="space-between" align="middle">
				<el-col :span="5">
					<el-row type="flex" justify="space-between">
						...
						<el-col :span="12">
							<el-button size="small" @click="importGoods">导入团购商品</el-button>
						</el-col>
					</el-row>
				</el-col>
        ...
        <el-form ref="form" :model="form" class="mod-form" v-if="form.tableData.length">
            <el-table ref="goodsTable" :data="form.tableData" style="width: 100%" v-loading="form.listLoading" :row-key="getKey">
			    ...
			    <el-table-column prop="groupNum" label="成团人数" min-width="120">
			    	<template scope="scope">
			    		<el-form-item :prop="'tableData.'+scope.$index+'.groupNum'"
			    				:rules="[
			    				{type:'number', required:true, min:3, message:'不能小于3人'},
			    				{validator:$validate.positiveInteger}]">
			    			<el-input type="number" v-model.number="scope.row.groupNum" size="small" :disabled="isView"></el-input>
			    		</el-form-item>
			    	</template>
			    </el-table-column>
			    <el-table-column prop="groupPrice" label="成团价格" min-width="150">
			    	<template scope="scope">
			    		<el-row type="flex" align="middle">
			    			<el-col :span="20">
			    				<el-form-item :prop="'tableData.'+scope.$index+'.groupPrice'"
			    				:rules="[
			    				{type:'number', required:true, message:'请输入成团价'},
		                		{type:'number', min:0.01, max:Number(scope.row.price), message:'需小于销售价'}]">
			    ...
			    <el-table-column prop="limitNumPer" label="每人限购数量" min-width="120">
			    	<template scope="scope">
			    		<el-form-item :prop="'tableData.'+scope.$index+'.limitNumPer'"
			    				:rules="[
			    				{type:'number', required:true, message:'请输入限购数量'},
			    				{validator:$validate.positiveInteger},
			    				{validator:$validate.stockValid, row:scope.row}]">
	...
	export default {
		...
		methods:{
			...
			getRedistableId(){
				...
				let opt = {
					actType: (self.activity && self.activity.actType) || 1
				}
			...
			setActMsg(){
				...
				if(this.activity){
					...
					this.actStartTime = util.formatDate.format(new Date(this.activity.activityTimeIntervals[0].effectStartTime), 'yy-MM-dd hh:mm:ss ');
					this.actEndTime = util.formatDate.format(new Date(this.activity.activityTimeIntervals[0].effectEndTime), 'yy-MM-dd hh:mm:ss ');
				}
			},
