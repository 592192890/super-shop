<template>
	<div class="mod-block mod-set">
		...
		<el-form ref="form" :model="form" class="mod-form" v-if="form.tableData.length">
			<el-table :data="form.tableData" v-loading="form.listLoading" :row-key="getKey">
				...
				<el-table-column prop="bn" label="SKU编码" min-width="250" align="left" show-overflow-tooltip>
				</el-table-column>
	...
	export default {
		...
		mounted(){
			...
			this.bus.$on('config/success', function(data){
				self.form.tableData[data.index].rule = data.data;
			});
