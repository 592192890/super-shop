		...
		<el-form-item v-for="(item, index) in form.xyPrice" :key="index" class="mod-item">
			...
			<div class="f-l" style="width: 100px">
				<el-form-item :prop="'xyPrice.' + index + '.value'" :rules="[{validator: $validate.required}, {validator: $validate.positiveFloat}]">
					<el-input v-model="item.value" size="small" class="num-box" @blur="formatPrice(item)"></el-input>
				</el-form-item>
			</div>
		...
		methods:{
			vXyPrice(){
				...
				for(var i=0;i<ar.length;i++){
					let item = ar[i];	
					let index = i;
					if(item.value>(item.num*this.popData.c2)){
						flag = false;
						self.xyPriceErrorMsg = '第'+(index+1)+'行数据错误,价格不能大于'+(item.num*this.popData.c2);
						break;
					}
					if(ar.length>1 && index>0){
						...
						if(!(Number(prev.num)<Number(item.num))||(!(Number(prev.value)<Number(item.value)))){
							flag = false;
							self.xyPriceErrorMsg = '第'+(index+1)+'行数据错误,件数递增,价格递增';
							break;
						}
					}
				}
