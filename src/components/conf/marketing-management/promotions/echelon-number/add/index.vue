	...
	export default {
		...
		methods:{
			...
			onSubmit(){
				...
				if(actType==1){
					setRouter = 'set-echelon-product';
				}
				if(actType==2){
					setRouter = 'set-number-product';
				}
				self.setActTypeName();
				this.$refs['form'].validate((valid) => {
					if(valid){
						sessionStorage.setItem('active-add',JSON.stringify(this.form));
						self.$router.push('/marketing-management/promotions/echelon-number/@'+setRouter+idStr);
					}
				});
			},
			onCancel(){
