    ...
    <div class="mod-block mod-set">
        ...
        <el-form ref="form" :model="form" class="mod-form" v-if="form.tableData.length">
            <el-table ref="goodsTable" :data="form.tableData" style="width: 100%" v-loading="form.listLoading" :row-key="getKey">
			    ...
			    <el-table-column prop="goodsName" label="商品名称" width="250" show-overflow-tooltip>
			    </el-table-column>
			    <el-table-column prop="name" label="规格" width="150" show-overflow-tooltip>
			    </el-table-column>
			    <el-table-column prop="bn" label="SKU编码" width="150" show-overflow-tooltip>
			    </el-table-column>
			    <el-table-column prop="price" label="销售价">
			    </el-table-column>
			    <el-table-column prop="cost" label="成本价">
			    </el-table-column>
			    <el-table-column prop="groupPrice" label="预售价" width="120">
			    	<template scope="scope">
	    				<el-form-item :prop="'tableData.'+scope.$index+'.groupPrice'"
	    				:rules="[
	    				{type:'number', required:true, message:'请输入预售价'},
                		{pattern: /^\d+(\.\d{1,2})?$/, message:'价格设置错误'},
                		{type:'number', min:0.01, max:Number(scope.row.price), message:'不能大于销售价'}]">
	    					<el-input type="number" v-model.number="scope.row.groupPrice" size="small" @blur="dataFilter(scope.$index, form.tableData)" :disabled="isView"></el-input>
	    				</el-form-item>
			    	</template>
			    </el-table-column>
			    <el-table-column prop="deposit" label="定金" width="120">
			    	<template scope="scope">
	    				<el-form-item :prop="'tableData.'+scope.$index+'.deposit'"
	    				:rules="[
	    				{type:'number', required:true, message:'请输入定金'},
                		{pattern: /^\d+(\.\d{1,2})?$/, message:'价格设置错误'},
                		{type:'number', min:0.01, max:Number(scope.row.groupPrice)-0.01, message:'需小于预售价'}]">
	    					<el-input type="number" v-model.number="scope.row.deposit" size="small" @blur="dataFilter(scope.$index, form.tableData)" :disabled="isView"></el-input>
	    				</el-form-item>
			    	</template>
			    </el-table-column>
			    <el-table-column prop="maxActStock" label="活动库存数" width="120">
			    	<template scope="scope">
			    		<el-form-item :prop="'tableData.'+scope.$index+'.maxActStock'"
			    				:rules="[
			    				{type:'number', required:true, message:'请输入活动库存'},
			    				{validator:$validate.positiveInteger}]">
			    			<el-input v-model.number="scope.row.maxActStock" size="small" :disabled="isView"></el-input>
			    		</el-form-item>
			    	</template>
			    </el-table-column>
			    <el-table-column prop="limitNumPer" label="每人限购数量" width="120">
			    	<template scope="scope">
			    		<el-form-item :prop="'tableData.'+scope.$index+'.limitNumPer'"
			    				:rules="[{validator:$validate.positiveInteger}]">
			    			<el-input v-model.number="scope.row.limitNumPer" size="small" :disabled="isView"></el-input>
			    		</el-form-item>
			    	</template>
			    </el-table-column>
		...
		methods:{
			...
			submitForm(formName) {
				...
				this.$refs[formName].validate((valid) => {
					if (valid) {
						self.onAddGoods(false, {data:self.form.tableData, key:'bn'}, function(){
              // 预售自提时间修改成后台接口对应的字段名称，取值来自 activity.selfZtTime 字段
              self.activity.sendEndTime = self.activity.selfZtTime[0].effectEndTime + (24*60*60-1)*1000;     // 发货/自提时间结束时间
			  self.activity.sendTime = self.activity.selfZtTime[0].effectStartTime;      // 发货/自提时间开始时间
              // 删除 activity.selfZtTime 字段后提交
              delete self.activity.selfZtTime;
			...
			getRedistableId(){
				...
				let opt = {
					actType: self.activity.actType || 1
				}
