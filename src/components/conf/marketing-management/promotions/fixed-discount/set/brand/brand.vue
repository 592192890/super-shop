			...
			<el-row type="flex" class="mod-tips" justify="space-between" align="middle">
				<el-col :span="5">
					<el-row type="flex" justify="space-between">
						<el-col :span="12">
							<el-button size="small" @click="addPartBrand">添加品牌</el-button>
						</el-col>
						<el-col :span="12">
							<el-button size="small" @click="importGoods">导入品牌折扣</el-button>
						</el-col>
					</el-row>
				</el-col>
...
<script>
	...
	export default {
		...
		methods:{
			...
			//添加商品
			addPartBrand(){
				this.bus.$emit('addBrand', {flag:true, sels:this.form.tableData, tableId: this.form.tableId, shopIds: this.activity?this.activity.shopIds:[]});
			},
			//分页事件触发
			handleCurrentChange(){
				let self = this;
			...
			historyBack(){
				let self = this;
				sessionStorage.setItem('active-set-brand', JSON.stringify(self.$data.form));
				this.$router.go(-1);
			},
			//活动提交
			submitForm(formName) {
			...
			getRedistableId(){
				let self = this;
				let tableId = sessionStorage.getItem('brand-tableId');
				let opt = {
				...
				!tableId && self.$ajax.post(self.$apiUrl.marketing.getRedistableId, {tableInfo: JSON.stringify(opt)}).then(function(res){
					...
					sessionStorage.setItem('brand-tableId', res.data);
				});
			},
			//设置活动信息
			setActMsg(){
		...
		mounted(){
			let self = this;
			// self.onAddGoods();
			this.form.tableId = sessionStorage.getItem('brand-tableId');
			self.setActMsg();
			self.getRedistableId();

			this.bus.$on('brand/submit', function(sels){
				self.onAddGoods(true, {data:sels, key:'brandId', transform:{name: 'brandName', id: 'brandId'}}, self.getRedistable)
			});
			...
			self.form.listLoading = true;
			if(sessionStorage.getItem('active-set-brand')){
				self.$data.form = JSON.parse(sessionStorage.getItem('active-set-brand'));
