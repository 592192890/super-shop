				...
				<div :class="isView?'block-mask':''">
					...
					<el-form-item label="会员设置" prop="resource" :rules="[{required: true, message: '请选择会员设置' }]">
						<el-radio-group v-model="form.resource" @change="resourceChange">
							<el-radio label="1" v-if="form.actType==1||form.actType==2||form.actType==3">所有用户</el-radio>
							<el-radio label="2" v-else>注册会员</el-radio>
						</el-radio-group>
					</el-form-item>
...
<script>
	...
	export default {
		...
		methods:{
			...
			setResource(){
				if(this.form.actType==4 || this.form.actType==5 || this.form.actType==6){
			...
			onSubmit(){
				let self = this, setRouter = null, actType = this.form.actType;
				let idStr = this.id?'?id='+this.id:'';
				if(actType==1 || actType==4){
					setRouter = 'set-goods'+idStr;
				}
				if(actType==2 || actType==5){
					setRouter = 'set-classify'+idStr;
				}
				if(actType==3 || actType==6){
					setRouter = 'set-brand'+idStr;
				}
				...
				this.$refs['form'].validate((valid) => {
					if(valid){
						...
						this.$router.push('/marketing-management/promotions/fixed-discount/@'+setRouter);
					}
				});
			},
			onCancel(){
		...
		created(){
			...
			this.$ajax.post('http://shopmanage.yunhou.com/api/active/type_list',{toolCode:this.form.toolCode}).then((res)=>{
				...
				// 	self.form.actType = res.data[0].value;
				// 	self.form.actTypeName = res.data[0].name;
				// }
			});
