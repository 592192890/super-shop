<template>
	<!-- 满额立减 begin  -->
	<div v-if="popData.offerContent=='1'">
		...
		<el-form-item v-for="(item, index) in form.xyPrice" :key="index" class="mod-item" v-if="!(index!=0 && form.isOverlying)">
			...
			<div class="f-l" style="width: 100px">
				<el-form-item :prop="'xyPrice.' + index + '.value'" :rules="[{validator: $validate.required}, {validator: $validate.positiveFloat}]">
					<el-input v-model.number="item.value" size="small" class="num-box" @blur="formatPrevPrice(item)"></el-input>
				</el-form-item>
			</div>
			<div class="text f-l">元减</div>
			<div class="f-l" style="width: 100px">
				<el-form-item :prop="'xyPrice.' + index + '.mljValue'" :rules="[
					{validator: $validate.required}, {validator: $validate.positiveFloat}, {validator: $validate.compare, max:item.quantity, message:'必须小于前面的价格'}
					]">
...
<script>
	export default {
		...
		methods:{
			vXyPrice(){
				if(this.popData.offerContent!='1'||this.form.isOverlying){
					return true;	
				}
				var flag = true;
				...
				for(var i=0;i<ar.length;i++){
					let item = ar[i];	
					let index = i;
					if(!(Number(item.value)>Number(item.mljValue))){
						flag = false;
						self.xyPriceErrorMsg = '第'+(index+1)+'行数据错误,优惠金额不能大于商品满减金额';
						break;
					}
					if(ar.length>1 && index>0){
						let prev = ar[index-1];	
						if((!Number(prev.value<item.value))||(!Number(prev.mljValue<item.mljValue))){
							flag = false;
			...
			onSubmit(){
				...
				// 		if(this.form.isOverlying){
				// 			this.delAr(this.form.xyPrice);
				// 		}
				// 		this.$ajax.post('http://t.com/success',this.form).then((res)=>{
				// 			this.$message.success('配置成功');	
				// 			this.bus.$emit('money-reduction/success');
				// 	});	
				// 	}
				// });
				this.$refs['form'].validate((valid) => {
					if(valid && this.vXyPrice()){
						...
						this.bus.$emit('money-reduction/success', data);
					}
				});
			},
			getPopData(){
