    ...
    <div class="mod-block mod-set">
        ...
        <el-form ref="form" :model="form" class="mod-form" v-if="form.tableData.length">
            <el-table ref="goodsTable" :data="form.tableData" style="width: 100%" v-loading="form.listLoading" :row-key="getKey">
			    <el-table-column label="序号" type="index" width="85" fixed="left">
			    </el-table-column>
			    ...
			    <el-table-column prop="promotionPrice" label="促销价格" min-width="180">
			    	<template scope="scope">
			    		<el-form-item :prop="'tableData.'+scope.$index+'.promotionPrice'"
			    				:rules="[
			    				{type:'number', required:true, message:'请输入促销价'},
			    				{type:'number', min:0.01, max:Number(scope.row.price), message:'需大于0小于等于销售价'},
			    				{validator:$validate.priceLength}]">
    						<el-input v-model.number="scope.row.promotionPrice" size="small" @blur="dataFilter(scope.$index, form.tableData)" :disabled="isView"></el-input>
    					</el-form-item>
			    	</template>
			    </el-table-column>
			    <el-table-column prop="maxActStock" label="活动库存数" min-width="150">
			    	<template scope="scope">
			    ...
			    <el-table-column prop="limitNumPer" label="每人限购数量" min-width="120" v-if="form.actType==3 || form.actType==5">
			    	<template scope="scope">
			    ...
			    <el-table-column prop="discount" label="折价码" min-width="120" v-if="form.actType==6">
			    	<template scope="scope">
			    		<el-form-item :prop="'tableData.'+scope.$index+'.discount'"
			    				:rules="[
			    				{type:'string', required: true, message:'请输入折价码'}]">
			    			<el-input type="string" v-model="scope.row.discount" size="small" :disabled="isView"></el-input>
			    		</el-form-item>
			    	</template>
			    </el-table-column>
	...
	export default {
		...
		methods:{
			...
			setActMsg(){
				...
				if(this.activity){
					...
					this.form.actType = this.activity.actType;
				}
			},
		...
		mounted(){
			...
			if(sessionStorage.getItem('active-set')){
				self.form = JSON.parse(sessionStorage.getItem('active-set'));
				this.form.actType = this.activity.actType;
