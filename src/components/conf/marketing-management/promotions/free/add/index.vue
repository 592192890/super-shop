			...
			<el-form ref="form" :model="form" label-width="140px">
				<div :class="isView?'block-mask':''">
					...
					<el-form-item label="活动类型" prop="actType" required>
						<el-col :span="7">
							<el-select v-model="form.actType" style="width:100%;" @change="selectChange">
	...
	export default {
		...
		methods:{
			...
			onSubmit(){
				...
				if(actType==1){
					setRouter = 'set-single-volume';
				}
				if(actType==2){
					setRouter = 'set-class-size';
				}
				self.setActTypeName();
				this.$refs['form'].validate((valid) => {
					if(valid){
						sessionStorage.setItem('active-add',JSON.stringify(this.form));
						self.$router.push('/marketing-management/promotions/free/@'+setRouter+idStr);
					}
				});
			},
			onCancel(){
		...
		//	this.$ajax.post('http://shopmanage.yunhou.com/api/active/type_list',{}).then((res)=>{
		//		self.select.userTags = res.data;
		//	});
