<template>
    <div class="sku">
       <step-header currentStep='2'></step-header>
                        ...
                        <el-col :span="6">
                            <el-form-item>
                                <el-select v-model="formEntity.countryId" filterable placeholder="请选择">
                                    <el-option label="请选择" value="" v-if="countryList.length"></el-option>
                            ...
                            <el-col :span="4">
                                <el-input placeholder="新增规格值" v-model="item.addedSpecName" class="add-sepc-input">
                                    <template slot="append">
                                        <img src="../../../../assets/images/conf/commodity-center/add-spec.png" @click="addSpec(item)" />
                                    </template>
                                </el-input>
                            </el-col>
                                ...
                                <tr v-for="item in specEntity" :key="item.specName">
                                    ...
                                    <td>
                                        <div class="cell" :span="2">
                                            <el-tooltip class="item" effect="light" content="唯一编号，必须按照商品实际信息准确填写；若商品由系统导入，则系统自动读取条码，无需填写。" placement="top">
                                                <el-input v-model="item.bn" v-bind:class="{error:item.isError_bn||item.isExist_bn||item.isDulplicate_bn}" @change="validateSpec(item,'bn',true)" :disabled="isFromRetek||isHasSpuId"></el-input>
                                            </el-tooltip>
                                        </div>
                                    ...
                                    <td>
                                        <div class="cell" :span="2">
                                            <el-tooltip class="item" effect="light" content="商品的唯一编号，必须按照商品实际信息准确填写；若商品由系统导入，则系统自动读取条码，无需填写。" placement="top">
                                                <el-input v-model="item.barCode" v-bind:class="{error:item.isError_barCode||item.isExist_barCode||item.isDulplicate_barCode}" @change="validateSpec(item,'barCode',true)" :disabled="isFromRetek||isHasSpuId"></el-input>
                                            </el-tooltip>
                                        </div>
                                    ...
                                    <td>
                                        ...
                                        <div :span="2" v-if="item.isError_cost" class="validate-error">
                                            <span>最多两位小数，整数位最多7个字符的非负数,小于销售价</span>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="cell" :span="2">
                                    ...
                                    <td>
                                        ...
                                        <div :span="2" v-if="item.isError_salePrice" class="validate-error">
                                            <span>最多两位小数,整数位最多7个字符的非负数,大于成本价</span>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="cell" :span="2">
                                            <el-input v-model="item.storeCount" v-bind:class="{error:item.isError_storeCount}"  @change="validateSpec(item,'storeCount')" :disabled="isFromRetek"></el-input>
                                        </div>
                                        <div :span="2" v-if="item.isError_storeCount" class="validate-error">
                                            <span>必填1-20个字符的整数</span>
                                        </div>
                                    </td>
                                    <!-- <td>
                                        <div class="cell" :span="2">
            ...
            <section>
                <div class="pic-info mod-toolbar">
                    ...
                    <el-row v-if="isClickGenerateNoSpecButton">
                        ...
                        <el-col :span="3" v-for="item in productPicArr" :key="item.imageId" style="width:100px;height:100px;position:relative;text-align:center;">
...
export default {
    ...
    data(){
        ...
        return {
            selectNoSpecRadio:false,//是否选中无规格radio
            ...
            isFromRetek:false,//判断是否来源于retek spu模板，如果是的话，一下字段不能编辑
            isHasSpuId:false,//判断 是否选择了spuId
    ...
    methods:{
                ...
                if(type=='weight'){
                    ...
                    if(/^[\d\w]{2,50}$/.test(item[type])){
                        ...
                        if(isQuery){
                            ...
                            bnTimer=setTimeout(()=>{
                                this.isExistDataBase('bn',item[type],item)
                            },300)
                        }
        ...
        generateSpec:function(){
            ...
            this.isClickGenerateNoSpecButton=false;//点击生成无规格
            //根据选择的规格生成需要显示多少规格图片
            // this.specPicArr=[];
        ...
        goSave:function(formName){
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    ...
                    //是否是retake下发的，如果是nostoreSell=0，否则nostoreSell=1
                    if(this.isFromRetek){
                      tempParams.skuVoList.forEach(v=>{
                        v.nostoreSell=0;
                      })
                    }else{
                      tempParams.skuVoList.forEach(v=>{
                    ...
                    this.$ajax.post(url,params).then(res=>{
                        if(res.error==0){
                            ...
                            this.$router.push({name:'商品发布3',params:{goodsId:res.data.goodsId}})
                        }else{
                            this.$message({
                                type:'error',
                                message:res.msg
        ...
        goBack:function(){
            this.$router.push({name:'商品发布1',params:{isBack:true}})
        },
        ...
        getDetail(backCateId,spuId){
            var url;
            if(spuId){
                this.isHasSpuId=true;
                url=this.$apiUrl.getSPUDetailUrl+"?backCateId="+backCateId+"&spuId="+spuId;
            }else{
                this.isHasSpuId=false;
                url=this.$apiUrl.getSPUDetailUrl+"?backCateId="+backCateId;
            }
            ...
            this.$ajax.post(url).then(res=>{
                if(res.error==0){
                    ...
                    if(!this.isHasSpuId){
                        this.formEntity.backcatId=this.selectedThirdCategory.catbackId;
                    }else{
                        if(res.data.spu.hasSpecs==1){
                        ...
                        //判断是否来源于spu模板
                        this.isFromRetek=res.data.spu.spuSource==1?true:false;


                        //将spu里面的属性改为商品相关的属性
                        res.data.spu.goodsName=res.data.spu.spuName;
                        res.data.spu.productName=res.data.spu.skuName;
                        res.data.spu.bn=res.data.spu.spuBn;
                        res.data.spu.storeCount=res.data.spu.storeCount||'0'

                        res.data.spu.pcGoodsInfo=res.data.spu.pcInfo||"";


                        this.formEntity=Object.assign({},res.data.spu);
                        ...
                        this.formEntity.backcatId=res.data.spu.catbackId;
                        //设置非自定义扩展属性的传递参数格式
                        var tempPropsText=[];
                        this.formEntity.propsText.forEach(v=>{
                            tempPropsText.push({id:v[0],value:v[1]})
                        })
                        this.formEntity.propsText=tempPropsText;
                        //设置自定义扩展属性的传递参数格式
                        var tempPropsAttributes=[];
                        if(!this.formEntity.propsAttributes){
                            this.formEntity.propsAttributes=[];
                        }
                        this.formEntity.propsAttributes.forEach(v=>{
                            tempPropsAttributes.push({id:v[0],value:v[3]})
                        })
                        this.formEntity.propsAttributes=tempPropsAttributes;


                        //商品图片
                        this.productPicArr=res.data.goodsImageList;
                        //tab标签内容
                        ...
                        this.formEntity.cateStr=categoryArr.join('>');


                        //是否显示规格sku数据
                        if(res.data.skuResultList&&res.data.skuResultList.length){
                            res.data.skuResultList.forEach(v=>{
                                if(this.isFromRetek){
                                ...
                                //判断数据库是否存在货号和条形码
                                this.isExistDataBase('barCode',v['barCode'],v);
                                this.isExistDataBase('bn',v['bn'],v);

                            })

                            //判断是否是无规格还是有规格
                            if(!res.data.skuResultList[0].specText||res.data.skuResultList[0].specText.length==0){
                                //无规格
                                this.selectNoSpecRadio=true;//选择无规格radio

                                //生成specName
                                res.data.skuResultList[0].specName='无规格';
                            }else{
                                //生成specName
                                res.data.skuResultList.forEach(v=>{
                                    var tempSpecNameArr=[];
                                ...
                                this.specArr.forEach(v=>{
                                    v.checked=[];
                                    res.data.skuResultList.forEach(vv=>{
                            ...
                            this.specEntity=res.data.skuResultList;
                        }

                        //自定义扩展属性
                        var propsAttributes=this.formEntity.propsAttributes;
                        if(propsAttributes){
                        ...
                        //非自定义扩展属性
                        var propsText=this.formEntity.propsText;
                        if(propsText){
                            this.unCustomAttributes.forEach((v,index)=>{
                                propsText.forEach(vv=>{
                                    if(v.id==vv.id){
                                        v.selected=parseInt(vv.value);//字符串转为数字
                        ...
                        res.data.spu.infoAttributes.forEach(v=>{
    ...
    mounted(){
        ...
        if(this.$route.params.entity){
            sessionStorage.setItem('selectedCategory',JSON.stringify(this.$route.params.entity))
        }
        var getSelectedCategory=JSON.parse(sessionStorage.getItem("selectedCategory"));
        this.getDetail(getSelectedCategory.backCateId,getSelectedCategory.spuId);
